...
describe("Onboarding", () => {
  beforeEach(() => {
    ...
    vi.mocked(checkSyncService).mockResolvedValue({
      status: 200,
      syncConfigured: true,
    ...
    vi.mocked(verifySyncConnection).mockResolvedValue({
      ...
      warnings: [],
      tasks: [{ id: "remote", text: "飞书任务", status: "todo", priority: "none", order: 0, remoteRecordId: "rec-remote" }],
    });
  });
  ...
  it("does not request a token when the reachable gateway is not configured", async () => {
    vi.mocked(checkSyncService).mockResolvedValueOnce({
      status: 200,
