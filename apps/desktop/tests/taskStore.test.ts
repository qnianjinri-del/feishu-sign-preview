...
describe("task store", () => {
  ...
  it("updates scheduling fields and rejects invalid reminders", () => {
    useTaskStore.getState().addTask("带提醒任务");
    const id = useTaskStore.getState().tasks[0].id;
    useTaskStore.getState().setTaskSchedule(id, {
      priority: "high",
      dueDate: "2026-08-12",
      dueTime: "18:00",
      reminderAt: new Date(2026, 7, 12, 17).toISOString(),
    });
    expect(useTaskStore.getState().tasks[0]).toMatchObject({ priority: "high", dueDate: "2026-08-12", dueTime: "18:00" });
    useTaskStore.getState().setTaskSchedule(id, {
      priority: "low",
      dueDate: "2026-08-12",
      dueTime: "09:00",
      reminderAt: new Date(2026, 7, 12, 10).toISOString(),
    });
    expect(useTaskStore.getState().tasks[0]?.priority).toBe("high");
    expect(useTaskStore.getState().toast?.message).toBe("提醒不能晚于截止时间");
  });

  it("updates appearance, shortcut, onboarding and reminder settings", () => {
    useTaskStore.getState().setListTitle("  发布清单  ");
    useTaskStore.getState().setOpacity(2);
    useTaskStore.getState().setCompactMode(true);
    useTaskStore.getState().setTheme("dark");
    useTaskStore.getState().setShortcuts("A", "B", "C");
    useTaskStore.getState().setOnboardingCompleted(true);
    useTaskStore.getState().setRemindersEnabled(false);
    expect(useTaskStore.getState().settings).toMatchObject({
      listTitle: "发布清单",
      opacity: 0.95,
      compactMode: true,
      theme: "dark",
      toggleWindowShortcut: "A",
      toggleClickThroughShortcut: "B",
      quickAddShortcut: "C",
  ...
  it("restores a task at a bounded index and supports redo", () => {
    useTaskStore.getState().addTasks(["一", "二"]);
    const removed = useTaskStore.getState().tasks[0];
    useTaskStore.getState().deleteTask(removed.id);
    useTaskStore.getState().restoreTask(removed, 99);
    expect(useTaskStore.getState().tasks.map((task) => task.text)).toEqual(["二", "一"]);
    useTaskStore.getState().undo();
  ...
  it("hydrates an equivalent persisted state", async () => {
    ...
    persisted.tasks = [{ id: "saved", text: "已保存", status: "done", priority: "none", order: 0, createdAt: "2026-01-01T00:00:00.000Z", updatedAt: "2026-01-01T00:00:00.000Z", completedAt: "2026-01-01T00:00:00.000Z", syncState: "pending" }];
