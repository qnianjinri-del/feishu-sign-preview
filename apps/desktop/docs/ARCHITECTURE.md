...
React components
      ↓ actions / selectors
Zustand task/settings/sync slices
      ↓ typed service calls
...
`src/stores/taskStore.ts` 装配 `src/stores/slices` 中的任务、设置和同步 action，并保存：
...
2. `migratePersistedState` 把 v1–v4 数据升级为 v5，校验 schema、修复损坏日期/提醒、转换旧 outbox 为 v2 mutation，并修复重复根 doing、同父项重复子 doing、孤立/过深层级和同级顺序。既有数据默认跳过首次向导。
...
2. `src/utils/sync.ts` 将前后快照转换为共享 `@floatlist/contracts` 定义的 v2 mutation。
...
服务端以飞书多维表格为权威数据源，负责稳定 ID 回填、根事项唯一“在干”、父行当前子事项摘要、父行隐藏 JSON 中的完整子事项数据、日期时区转换、父子约束、软归档、幂等键和单实例写入串行化。飞书主表只创建根事项物理行，子事项由同步层展开为普通 `Task`，因此不会在主表重复占行。桌面端从不直接访问飞书开放平台。

## 日期和系统提醒

`src/utils/taskSchedule.ts` 负责日期边界、逾期判断、预设提醒和组合筛选，所有逻辑均可在浏览器外单测。`src/services/notifications.ts` 是 Tauri 通知插件的唯一前端边界：任务完成、删除或时间变化后会取消并重建尚未触发的系统通知。拒绝通知权限不会删除 `reminderAt`，界面会显示未授权状态并允许重新检查。
