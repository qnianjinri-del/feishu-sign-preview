...
- `⌘F` 统一按状态、日期、优先级和关键词筛选，筛选时自动禁用拖拽排序
- 父子事项均支持优先级、截止日期/时刻与 macOS 原生提醒
...
- macOS 13.5 或以上，Apple Silicon 或 Intel
- Node.js 22+
- pnpm 11+
- Rust stable
...
以上命令从仓库根目录运行。仅调试 React 界面可运行 `pnpm dev:desktop`；完整原生调试运行 `pnpm tauri dev`。浏览器模式使用内存存储，只有 Tauri 模式会使用正式本地 Store 和原生窗口 API。
...
测试覆盖事项增删改、父子层级、四态切换、组合筛选、日期边界、优先级、原生提醒、快速新增、首次向导、受阻原因、根事项唯一正在做、子状态推进、父项完成约束、撤销、排序、同步 v2 mutation/outbox、远端快照与冲突、hydrate、v1–v4 → v5 迁移修复，以及关键 UI 键盘交互。
...
```text
apps/desktop/src-tauri/target/release/bundle/macos/FloatList.app
apps/desktop/src-tauri/target/release/bundle/dmg/FloatList_0.3.0_aarch64.dmg
```
...
正式数据由 `@tauri-apps/plugin-store` 写入应用数据目录中的 `floatlist.json`。当前 schema 为 v5；v1–v4 会在加载时迁移并保留任务 ID，v5 新增优先级、截止日期/时刻、提醒与 v2 outbox。迁移会修复无效日期、孤立或过深层级。保存使用 220ms 防抖，正常退出前会主动 flush。无法解析的 Store 文件会先重命名为 `floatlist.corrupt-<timestamp>.json`，再以默认数据启动。
...
首次连接且本地、飞书两边都有事项时，应用不会自动覆盖任何一边，而是要求选择“采用飞书”或“合并本地事项”。断网时本地操作继续可用，变更进入持久化 outbox，恢复连接后按顺序重试。v0.3 桌面端只兼容同步协议 v2；部署时先升级网关 2.0。服务端部署和环境变量参见 [仓库根目录 README](../../README.md)。

如果同步服务运行在本机 `127.0.0.1:3000`，可执行 `pnpm --filter @floatlist/gateway install:sync-gateway` 安装 macOS 登录启动项。FloatList 自身的“开机启动”只负责启动桌面应用，不会自动启动 Node 同步网关。
