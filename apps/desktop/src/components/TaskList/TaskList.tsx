...
export function TaskList({ onAdd, filterMode = "current", query = "", dateFilter = "any", priorityFilter = "any" }: TaskListProps) {
  ...
  const groups = useMemo(() => filterTaskGroups(tasks, {
    ...
    dateFilter,
    priorityFilter,
  }), [dateFilter, filterMode, priorityFilter, query, showCompleted, tasks, todayStart]);
  const sortingDisabled = filterMode !== "current" || Boolean(query.trim()) || dateFilter !== "any" || priorityFilter !== "any";
