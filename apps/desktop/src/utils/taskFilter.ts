...
export function filterTaskGroups(tasks: Task[], options: TaskFilterOptions): TaskGroup[] {
  ...
  const priorityFilter = options.priorityFilter ?? "any";
  const now = options.now ?? new Date();
  const matchesActiveFilters = (task: Task) => visibleInMode(task, options.mode, options.showCompleted)
    && matchesQuery(task, query)
    && matchesTaskDateFilter(task, dateFilter, now)
    && taskPriorityMatches(task, priorityFilter);
  const roots = tasks
  ...
  return roots.flatMap((root) => {
    ...
    const matchingChildren = allChildren.filter(matchesActiveFilters);
    const rootMatches = matchesActiveFilters(root);
