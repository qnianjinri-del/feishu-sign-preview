...
import type { SyncTask, TaskPriority, TaskStatus } from "../types/task-sync.js";
...
interface BitableTaskRepositoryOptions {
  ...
  dueTimeFieldName?: string;
  fetchFn?: FetchLike;
  orderFieldName?: string;
  parentIdFieldName?: string;
  priorityFieldName?: string;
  reminderAtFieldName?: string;
...
function normalizeCalendarDate(value: unknown, timeZone: string): string | undefined {
  const milliseconds = typeof value === "number" ? value : typeof value === "string" ? Number(value) : Number.NaN;
  if (!Number.isFinite(milliseconds)) return undefined;
...
function calendarDateToTimestamp(value: string, timeZone: string): number {
...
function priorityToRemote(value: TaskPriority): string | null {
...
function parseEmbeddedSubtasks(
  ...
  try {
    ...
    return parsed.flatMap((item, index) => {
      ...
      const dueTime = normalizeText(item.dueTime);
      const reminderAt = optionalIso(item.reminderAt);
...
export class BitableTaskRepository implements TaskRepository {
  ...
  constructor(options: BitableTaskRepositoryOptions = {}) {
    this.options = {
      ...
      dueDateFieldName: options.dueDateFieldName ?? config.bitableDueDateFieldName,
      dueTimeFieldName: options.dueTimeFieldName ?? config.bitableDueTimeFieldName,
      ...
      priorityFieldName: options.priorityFieldName ?? config.bitablePriorityFieldName,
      reminderAtFieldName: options.reminderAtFieldName ?? config.bitableReminderAtFieldName,
      ...
      timeZone: options.timeZone ?? config.floatlistTimeZone,
    };
  }
  ...
  async updateTask(task: RepositoryTask, patch: TaskPatch): Promise<void> {
    ...
    if (task.parentId) {
      ...
      if (patch.dueDate !== undefined) {
        if (patch.dueDate) nextTask.dueDate = patch.dueDate;
        else delete nextTask.dueDate;
      }
      if (patch.dueTime !== undefined) {
        if (patch.dueTime && (patch.dueDate ?? nextTask.dueDate)) nextTask.dueTime = patch.dueTime;
        else delete nextTask.dueTime;
      }
      if (patch.reminderAt !== undefined) {
        if (patch.reminderAt) nextTask.reminderAt = patch.reminderAt;
    ...
    if (patch.priority !== undefined) fields[this.options.priorityFieldName] = priorityToRemote(patch.priority);
    if (patch.dueDate !== undefined) {
      fields[this.options.dueDateFieldName] = patch.dueDate
        ? calendarDateToTimestamp(patch.dueDate, this.options.timeZone)
        : null;
    }
    if (patch.dueTime !== undefined) fields[this.options.dueTimeFieldName] = patch.dueTime ?? "";
    if (patch.reminderAt !== undefined) {
      fields[this.options.reminderAtFieldName] = patch.reminderAt ? Date.parse(patch.reminderAt) : null;
    }
  ...
  private async mapRecord(
    ...
    const priority = normalizePriority(fields[this.options.priorityFieldName]);
    const dueDate = normalizeCalendarDate(fields[this.options.dueDateFieldName], this.options.timeZone);
    const dueTime = normalizeText(fields[this.options.dueTimeFieldName]);
    const reminderAt = normalizeRemoteTime(fields[this.options.reminderAtFieldName]);
  ...
  private toRemoteFields(task: SyncTask, extra: { archived: boolean }): Record<string, unknown> {
    const fields: Record<string, unknown> = {
      ...
      [this.options.priorityFieldName]: priorityToRemote(task.priority),
      [this.options.dueDateFieldName]: task.dueDate
        ? calendarDateToTimestamp(task.dueDate, this.options.timeZone)
        : null,
      [this.options.dueTimeFieldName]: task.dueTime ?? "",
      [this.options.reminderAtFieldName]: task.reminderAt ? Date.parse(task.reminderAt) : null,
  ...
  private fieldNames(): string[] {
    return [
      ...
      this.options.dueDateFieldName,
      this.options.dueTimeFieldName,
      this.options.priorityFieldName,
      this.options.reminderAtFieldName,
    ];
  }
