...
fn sync_tray_state(
    ...
    state.always_on_top.set_checked(always_on_top).map_err(|error| error.to_string())?;
    state.click_through.set_checked(click_through).map_err(|error| error.to_string())?;
    state.launch_at_login.set_checked(launch_at_login).map_err(|error| error.to_string())?;
    Ok(())
}
...
fn backup_corrupt_store(app: AppHandle) -> Result<Option<String>, String> {
    let source = app.path().app_data_dir().map_err(|error| error.to_string())?.join("floatlist.json");
    if !source.exists() {
        return Ok(None);
    }
...
fn build_tray(app: &tauri::App) -> tauri::Result<()> {
    ...
    let launch_at_login = CheckMenuItem::with_id(app, "launch_at_login", "开机启动", true, false, None::<&str>)?;
