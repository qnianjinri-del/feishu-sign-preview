...
fn validate_service_url(value: &str) -> Result<Url, String> {
    ...
    let host = url.host_str().ok_or_else(|| "同步服务地址缺少主机名".to_string())?;
...
pub async fn sync_delete_client_token() -> Result<(), String> {
    run_keychain(|| match find_generic_password(None, KEYCHAIN_SERVICE, KEYCHAIN_ACCOUNT) {
        Ok((_, item)) => {
            item.delete();
            Ok(())
        }
