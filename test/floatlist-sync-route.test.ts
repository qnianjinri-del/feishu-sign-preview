...
test("FloatList snapshot route requires authentication and supports ETag", async () => {
  ...
  try {
    const unauthorized = await app.inject({ method: "GET", url: "/api/floatlist/v1/tasks" });
...
test("FloatList mutation route validates idempotency and returns authoritative state", async () => {
  ...
  try {
    const missingKey = await app.inject({
      method: "POST",
      url: "/api/floatlist/v1/mutations",
      headers: { authorization: "Bearer test-client-token" },
      payload: { operations: [{ operationId: "block", type: "set_blocked", taskId: "one", reason: "等待权限" }] },
    });
...
test("FloatList sync routes rate limit repeated clients", async () => {
  ...
  try {
    ...
    const first = await app.inject({ method: "GET", url: "/api/floatlist/v1/tasks", headers });
    const limited = await app.inject({ method: "GET", url: "/api/floatlist/v1/tasks", headers });
    assert.equal(first.statusCode, 200);
